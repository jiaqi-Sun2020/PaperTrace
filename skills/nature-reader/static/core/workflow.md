...
- perform the translation pass directly as Codex by default; do not stop to look for local/third-party translation models unless the user asked for them
...
Do not treat a caption-only text block or an image-object list as a completed figure/table. The final `paper.md` must contain a card with the visual or semantic table itself.
...
Do not add an interactive Q&A panel or follow-up widget in the Markdown deliverable. Browser output belongs to the validated `reader_interactive.html` step after the Markdown bundle is fully translated and structurally complete.
