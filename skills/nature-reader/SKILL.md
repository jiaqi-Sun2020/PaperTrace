...
- For a full paper, author `reader_wiki/concept_candidates.json` with paper-specific concepts, exact source anchors/evidence spans, Chinese aliases, types, and non-template explanation notes. Formula numbers, section numerals, layout tokens, and generic uppercase words are not concepts.
