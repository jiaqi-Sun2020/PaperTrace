#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Extract reviewable PaperTrace profile signals from exported chat sessions."""
...
SENSITIVE_VALUE_RE = re.compile(
    r"(sk-[A-Za-z0-9_-]{16,}|Bearer\s+[A-Za-z0-9._-]{20,}|api[_-]?key\s*[:=]\s*['\"]?[A-Za-z0-9._-]{16,}|password\s*[:=]\s*['\"]?[^'\"\s]{8,})",
    re.I,
)
...
def concept_id_from_label(label: str) -> str:
    abbreviations = re.findall(r"\(([A-Za-z0-9_.+-]{2,24})\)", label)
    if abbreviations:
        return slugify(abbreviations[-1], "concept")
    return slugify(label, "concept")
...
def content_parts_to_text(content: Any) -> str:
    ...
    if isinstance(content, dict):
        ...
        if isinstance(content.get("fragments"), list):
            return "\n".join(content_parts_to_text(row) for row in content["fragments"])
        if content.get("type") == "thinking" and content.get("thinking"):
            return "(Thinking)\n" + content_parts_to_text(content["thinking"])
        if content.get("type") == "tool_use":
            return "(Tool Use)\n" + clean_text(content.get("name") or content.get("id") or "tool_use", 500)
        if content.get("content"):
            return content_parts_to_text(content["content"])
        if content.get("text"):
            return content_parts_to_text(content["text"])
        if content.get("value"):
            return content_parts_to_text(content["value"])
    return ""


def message_model(message: dict[str, Any]) -> str:
    metadata = message.get("metadata") if isinstance(message.get("metadata"), dict) else {}
    return clean_text(metadata.get("model_slug") or metadata.get("model") or message.get("model"), 120)
...
def messages_from_claude(data: dict[str, Any], title: str, source_path: Path) -> list[dict[str, Any]]:
    ...
    for message in chat_messages:
        if not isinstance(message, dict):
            continue
        sender = clean_text(message.get("sender") or message.get("role") or "unknown", 80).lower()
        ...
        rows.append({
            "title": title,
            "role": role,
            "timestamp": clean_text(message.get("created_at") or message.get("updated_at"), 120),
            "model": clean_text(message.get("model") or "Claude", 120),
            "text": text,
            "source_path": str(source_path),
        })
    return rows
...
def messages_from_json(data: Any, source_path: Path) -> list[dict[str, Any]]:
    ...
    if isinstance(data, dict):
        ...
        rows.extend(messages_from_claude(data, title, source_path))
        if rows:
            return rows
...
def split_text_messages(text: str, source_path: Path) -> list[dict[str, Any]]:
    ...
    def flush() -> None:
        ...
        body = clean_text("\n".join(current), 12000)
        if body:
            messages.append({"title": source_path.stem, "role": current_role, "timestamp": "", "model": "", "text": body, "source_path": str(source_path)})
        current = []
    ...
    return [{"title": source_path.stem, "role": "unknown", "timestamp": "", "model": "", "text": chunk, "source_path": str(source_path)} for chunk in chunks if chunk.strip()]
...
def event_topic_tags(text: str) -> list[str]:
    tags: list[str] = []
    for term in CONCEPT_HINTS + RESEARCH_HINTS:
        if term.lower() in text.lower():
            tag = slugify(term, "topic")
            if tag not in tags:
                tags.append(tag)
    for raw in re.findall(r"\b[A-Z][A-Za-z0-9-]{2,24}\b", text):
        tag = slugify(raw, "topic")
        if tag not in tags:
            tags.append(tag)
    return tags[:12]


def source_summary(source: dict[str, Any], events: list[dict[str, Any]]) -> dict[str, Any]:
    user_events = [row for row in events if row.get("role") == "user"]
    all_text = "\n".join(row.get("text", "") for row in events)
    questions = [clean_text(row.get("text"), 220) for row in user_events if "?" in row.get("text", "") or "？" in row.get("text", "")]
    preferences = []
    for row in user_events:
        text = row.get("text", "")
        if re.search(r"(我希望|我喜欢|优先|默认|记住|first principles|第一性原理|直接|具体|可执行)", text, re.I):
            preferences.append(clean_text(text, 240))
    actions = []
    for row in user_events:
        text = row.get("text", "")
        if re.search(r"(请|帮我|修改|生成|实现|修复|整理|导入|同步|审核)", text, re.I):
            actions.append(clean_text(text, 240))
    topic_tags: list[str] = []
    for tag in event_topic_tags(all_text):
        if tag not in topic_tags:
            topic_tags.append(tag)
    first_user = clean_text(user_events[0].get("text"), 260) if user_events else ""
    return {
        "source_id": source["source_id"],
        "title": source["title"],
        "path": source["path"],
        "at_a_glance": first_user or clean_text(all_text, 260),
        "topic_tags": topic_tags,
        "explicit_preferences": preferences[:8],
        "open_questions": questions[:8],
        "action_like_requests": actions[:8],
        "event_count": len(events),
        "user_event_count": len(user_events),
        "models": sorted({row.get("model", "") for row in events if row.get("model")}),
    }
...
def cmd_collect(args: argparse.Namespace) -> int:
    ...
    for path in files:
        ...
        for turn_index, message in enumerate(messages, start=1):
            ...
            event = {
                ...
                "model": clean_text(message.get("model"), 120),
                "text": text,
        ...
        summaries.append(source_summary(source, source_events))
    write_jsonl(output_dir / "sources.jsonl", sources)
    write_jsonl(output_dir / "events.jsonl", events)
    write_json(output_dir / "conversation_summaries.json", {
        "summary_version": 1,
    ...
    manifest = {
        ...
        "sensitive_events_skipped": sensitive_events_skipped,
    ...
    print(f"Wrote {output_dir / 'conversation_summaries.json'}")
    print(f"Sources: {len(sources)}")
    print(f"Events: {len(events)}")
    if skipped:
        print(f"Skipped URL inputs: {len(skipped)}")
    if sensitive_events_skipped:
        print(f"Sensitive-looking events skipped: {sensitive_events_skipped}")
    return 0
...
def workflow_candidates(event: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if re.search(r"(reader-skill|lean-html-skill|nature-reader|reader-learner|knowledge_profile|画像)", text, re.I):
        items.append(base_candidate("workflow_preference", "modular PaperTrace skill chain", event, 0.8, "User works through modular PaperTrace skills and learner profile."))
...
def cmd_apply(args: argparse.Namespace) -> int:
    ...
    scripts_dir = project_root / "skills" / "reader-learner" / "scripts"
    sys.path.insert(0, str(scripts_dir))
    from profile_v2 import save_json  # type: ignore

    save_json(profile_path, profile, backup=False)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    apply.add_argument("--project-root", help="Project root. Defaults to the PaperTrace root inferred from profile path.")
