...
def compact_continuing_item(item: dict[str, Any], prior: dict[str, Any] | None, item_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "source_class": clean_text(item.get("source_class"), 120),
        "organization": clean_text(item.get("organization"), 200),
        "topic": clean_text(item.get("topic"), 200),
        "corroborating_source_count": item.get("corroborating_source_count", 0),
        "ranking_signals": item.get("ranking_signals") if isinstance(item.get("ranking_signals"), dict) else {},
