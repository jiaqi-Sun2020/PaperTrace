...
- Every new release requires one concealed-name causal opening story and one complete, concept-appropriate worked example before the briefing body. Fable Mode begins with at least two causally necessary background paragraphs and has no total paragraph-count cap; quantitative narratives must run the same concrete inputs, operations, results, counterfactual, and observable difference later formalized by the worked example. Each paragraph is limited to 2000 normalized characters and is rejected rather than truncated. Select the topic for learner relevance, causal value, and source support rather than equation availability; use formulas only when the mechanism genuinely requires them, never to satisfy the schema. HTML must keep the example in a native `details` control closed by default. Story/example exposure is not learner evidence.
