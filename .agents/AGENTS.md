...
- Every new release requires a complete source-grounded Fable (`opening_story.version=3`) and a consistent worked example. Follow `skills/allegory-teach/references/daily-briefing-interface.md`. At least two causal background paragraphs precede the mechanism; no total length/count ceiling applies. Keep the story fully visible, reveal the concept only in the factual return, and collapse only the formal example. Historical versions 1–4 remain compatible; exposure never establishes knowledge.
