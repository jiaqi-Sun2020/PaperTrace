...
3. `reader-learner` imports user feedback from HTML or natural language and updates a long-term learner profile under `.agents/reader-learner/knowledge_profile.json`.
...
6. `init-knowledge-profile` imports local GPT/ChatGPT conversation exports through a staged `collect -> extract -> propose -> apply` workflow, so prior conversations can initialize or extend the learner/person profile without unreviewed profile mutation.
...
- Python scripts for GPT conversation import staging and profile patch generation.
...
- `skills/utils/init-knowledge-profile/SKILL.md`
...
8. Use `reader-learner` to import the JSON or pasted payload into `.agents/reader-learner/knowledge_profile.json`.
...
6. Import exported `news_feedback.json` with `skills/ai-quantum-news-briefing/scripts/import_news_feedback.py`; that script normalizes to reader-feedback shape and delegates profile mutation to `reader-learner`.

## GPT Conversation Import Workflow

1. Save or export ChatGPT/GPT conversations as local `.txt`, `.md`, `.html`, or `.json` files. Do not depend on share URL fetching for reproducibility.
2. Run `skills/utils/init-knowledge-profile/scripts/init_knowledge_profile.py collect` to write `sources.jsonl`, `events.jsonl`, and `manifest.json` under `.agents/reader-learner/imports/chat_sessions/`.
...
5. Review the patch. Only then run `apply --backup`, which sends concept-status candidates through the existing `reader-learner` importer and writes non-concept user traits under `person_profile`.
...
GPT conversation imports update the learner/person profile only through a reviewed `profile_patch.json`. Concept exposure from chat alone should remain `unrated` unless the user explicitly expressed understanding or confusion.
