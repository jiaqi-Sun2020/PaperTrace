...
5. `allegory-teach` turns one selected, profile- or source-grounded concept into the least elaborate explanation that repairs the missing bridge. Technical correctness and beginner comprehensibility are independent hard gates. Bridge Mode runs a concrete case first and uses a local analogy only when it lowers cognitive load. An explicit Fable request first narrows and repairs one relation, then falls back transparently to Bridge when the story still only renames variables; daily openings remain Fable and use the ranked explicit-override path when rank 1 cannot support a valid story. A fable must couple the actor's goal, real limit, choice/action, and consequence to the technical objective, rule source, inputs, limit, and result. It completes both spines before compression, has no total length cap, and treats the daily 2000-character value as a per-paragraph transport boundary. Its factual debrief follows `1 -> 2 -> 3 -> 4`, and the worked example compactly formalizes rather than substitutes for the story.
...
- `2026/7/A Quantum-Walk Representation of Color-Ordered MHV Scattering Amplitudes_reader/paper.md`
- `2026/7/A Quantum-Walk Representation of Color-Ordered MHV Scattering Amplitudes_reader/reader_interactive.html`
...
6. Read the HTML with the source/article/Contents panes sized or folded as needed. Click highlighted concepts or use free annotation, then click `Save mark` for each feedback item. The annotation workspace must leave translated content reachable; saved annotations appear as badges near the source block and in a deletable list inside the feedback panel.
7. Reader marks and unfinished input are automatically recoverable in the same browser. At the end of the reading session, manually export all saved feedback with `Download feedback JSON` or `Copy feedback for Codex` for a portable handoff.
...
6. Author one concealed-name causal opening story and one complete worked example from the ranked source set or stable learner boundary. Use at least two causally necessary background paragraphs in the simplest familiar setting that preserves the mechanism and as many mechanism paragraphs as needed. Reject story terms that require a second explanation. Quantitative stories must expose the actual inputs, operations, results, comparison, and observable difference later formalized compactly by the worked example. Render the factual debrief in `1 -> 2 -> 3 -> 4` order. Select the concept for relevance and explanatory value, not equation availability; use formulas only when the mechanism genuinely requires them.
...
Interactive reader HTML does not automatically write to `.agents`. It keeps a source-map-SHA-256-isolated browser recovery copy of saved marks and unfinished form input, so reopening the same reader can restore an interrupted session. The user must still manually export/copy feedback for portability, then import it with `reader-learner` or ask Codex to process the copied payload. Browser recovery never implies profile import.
...
## Current Verified Local Examples

```text
2026/7/A Quantum-Walk Representation of Color-Ordered MHV Scattering Amplitudes_reader/
|-- paper.md
|-- source_map.json
...
news/2026-07-16/
|-- daily_briefing_2026-07-16.md
|-- briefing_reader_2026-07-16.html
|-- news_feedback_2026-07-16.json
|-- news_feedback_config_delta_2026-07-16.json
|-- daily_pipeline_manifest_2026-07-16.json
`-- daily_pipeline_index_updates_2026-07-16.json
```

The 2026-07-16 briefing contains 8 academic and 12 social-news items with 60 default-`unrated` concepts and a `complete` strict-verification manifest. Generated reader/news artifacts are local and Git-ignored; `CHANGES.md` records the durable release facts.
