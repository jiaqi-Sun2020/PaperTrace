...
5. `allegory-teach` turns one selected, profile- or source-grounded concept into a bridge-first explanation or, when explicitly requested or used by the daily briefing, a concealed-name causal fable. Fables use adaptive length, run a checkable concrete case in the narrative itself, and return through the `1 -> 3 -> 4 -> 2` factual debrief plus the same concept-appropriate worked example. Topic selection is independent of equation availability; formulas are allowed only when the mechanism genuinely requires them.
...
6. Author one concealed-name causal opening story and one complete worked example from the ranked source set or stable learner boundary. Use at least two causally necessary background paragraphs and as many mechanism paragraphs as needed; quantitative stories must expose the actual inputs, operations, results, comparison, and observable difference also used by the worked example. Select the concept for relevance and explanatory value, not equation availability; use formulas only when the mechanism genuinely requires them.
