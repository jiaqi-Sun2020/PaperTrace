...
- **Pipeline 2 — AI + Quantum Daily Briefing Release:** `ai-quantum-news-briefing` owns discovery, evidence eligibility, `news-ranker-v1` scoring/quota selection, validation/rendering of the `allegory-teach` opening-story handoff, Delta compaction, staging, verification, publication, feedback parity, manifest, and story-index commit. Candidate/config/Markdown alone are not terminal artifacts.
...
- `nature-reader`: source-paper to internal bilingual evidence/bundle with faithful `中文` translations plus logic, knowledge-point, formula, figure, and reading guidance in `注释`. It owns atomic formula reconstruction, page-reviewed source-math inventories, and complete source-language Algorithm LaTeX/compile assets. The active current-session primary model directly authors Chinese, notes, and LaTeX; external/secondary translation backends are not content authors.
- `reader-skill`: reader-specific Markdown parsing, translation validation, source anchors, bilingual body semantics, source-math-inventory validation, compiled-Algorithm/hash verification, atomic-formula/dedup gates, and learner-profile annotation metadata. It may keep compatibility HTML wrappers, but reusable HTML shell, feedback UI, and export controls should move to `lean-html-skill`. `reader_interactive.html` is only for completed translated readers; incomplete bundles must be fixed before HTML generation.
...
- `allegory-teach`: optional read-only narrative explanation layer inside Pipeline 4 and authoring dependency for Pipeline 2. It converts one selected concept into a concealed-name causal fable, factual debrief, and a mathematical, numerical, operational, causal, experimental, or comparative worked example without owning topic ranking, publication, or learner-state mutation.
...
- `allegory-teach` owns only narrative authoring, factual debrief, and the concept-appropriate worked example. It must not prefer a topic because it admits equations, fabricate formulas, mutate the profile, alter ranking, or publish the daily briefing.
...
- Figure/table extraction and formula reconstruction are structural reader layers, not optional decoration. A final reader needs inspectable figure/table cards, semantic tables where appropriate, and LaTeX-readable formulas. Any source row with math-layout evidence carries an ordered, page-reviewed source-math inventory; both language fields must render every inventory component exactly.
- Algorithm extraction is also a structural reader layer. A final reader needs one full source-language Algorithm card compiled from LaTeX; only actual source comments may be translated inside the card. Algorithm summaries and translated duplicate bodies are invalid.
