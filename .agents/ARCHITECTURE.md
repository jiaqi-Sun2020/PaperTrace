...
- `reader-learner`: learner profile import/update commands.
...
- `utils/init-knowledge-profile`: reviewable chat-session import layer for initializing or extending `knowledge_profile.json` from exported GPT conversations through sources, evidence events, candidates, and patches.
...
- `init-knowledge-profile` owns GPT conversation import staging. It may generate concept-status handoff feedback for `reader-learner`, but it should not silently overwrite the learner/person profile without a reviewable patch and backup.
