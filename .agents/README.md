...
- `skills/allegory-teach`: optional read-only explanation layer for one selected advanced concept and the Pipeline 2 opening-story handoff. Technical correctness and beginner comprehensibility are hard gates. It defaults to a concrete case and uses a local analogy only when that lowers cognitive load; explicit and daily Fable Mode uses a minimal familiar setting, adaptive-length causal background, a checkable in-story case, a `1 -> 2 -> 3 -> 4` factual debrief, and a compact formalization of the same case without changing ranking, feedback, or learner state.
