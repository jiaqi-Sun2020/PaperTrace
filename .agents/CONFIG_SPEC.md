...
For PDF inputs, `source_map.pages` is the immutable page-view manifest. Each row contains a positive `page`, a bundle-relative `assets/source_pages/...` image, and its SHA-256. Formal HTML renders one enlarged, viewport-height synchronized page viewer in the left column, the article in the center, and sticky Contents in the right column on wide screens; it adds `data-source-page` to source blocks and supplies independent Original/source-page collapse controls. Narrow screens may reflow these regions without dropping them. Absolute paths, traversal, missing/hash-stale pages, full-page figure cards, inaccessible controls, misplaced Contents, or print-hidden Original panels fail publication.
