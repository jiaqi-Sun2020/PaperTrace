...
| 理解一个研究边界附近的高阶机制 | [Allegory Teach](#allegory-teach) | 完整寓言、故事后的事实回扣，以及一个一致的具体实例 |
...
> 阅读当前项目下的 README 和 <code>.agents</code>，根据 <code>&lt;日期或日期范围&gt;</code> 的当前、可核验来源，完成 AI + Quantum 日报发布流水线。先构建带有隐藏概念名称的开篇故事和一个适配概念类型的完整例子的有证据候选配置，再依次完成 <code>run → verify → finalize → verify</code>。交付可交互日报 HTML、反馈 JSON、manifest 与 index；候选清单或 staging 输出都不能当作最终结果。
...
- `allegory-teach` 的普通解释、生活类比和日报开篇统一使用完整 Fable。技术正确、证据忠实和初学者可理解是独立硬门；先保证目标、限制、规则、选择和后果完整，再删除装饰与重复，不设总字数或段数上限。
...
`skills/allegory-teach` 是自适应教学的解释层和日报开篇创作依赖。所有新调用统一 Fable，新日报使用 version 3；至少两个必要背景段，不设总篇幅上限，单段 2000 字符仅是传输边界。故事全文展开，事实回扣按 `1 → 2 → 3 → 4`，同一案例的技术例子默认折叠。Bridge、Everyday 仅保留为历史兼容文档，既有 v1–v4 仍可读取。先缩小关系并修复；独立请求仍失败时报告障碍，不自动降级；日报可沿排名显式覆盖来源，没有合格来源则停止发布。

> 使用 `$allegory-teach` 从我的当前研究边界选择一个概念，用完整而忠实的寓言讲清一个关系；故事结束后才揭名，按 `1 → 2 → 3 → 4` 回到事实，再给出同一案例的具体例子。
