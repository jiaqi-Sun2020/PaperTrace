...
- <code>allegory-teach</code> treats technical correctness and beginner comprehensibility as independent hard gates. Direct teaching starts with the real problem and a minimum concrete case, adding one local analogy only when it reduces cognitive load. An explicit allegory request or daily opening story uses a concealed-name fable with at least two causally necessary setup paragraphs in the simplest familiar setting that works, followed by as many mechanism paragraphs as the logic requires. Quantitative fables run their actual inputs, operations, results, counterfactual, and observable difference inside the story; non-mathematical fables use explicit states without decorative formulas.
...
<code>skills/allegory-teach</code> is the optional beginner-first companion to Adaptive Teach and the read-only authoring dependency for the daily briefing's opening story. Technical correctness and beginner comprehensibility are hard gates; literary style is optional. Bridge Mode is the ordinary first-contact path: disclose the real problem, run a minimum concrete case, and add one mapped local analogy only when the case leaves a relation unclear and the analogy lowers cognitive cost. Fable Mode is reserved for an explicit story/delayed-reveal request and for daily openings; every new fable starts with at least two causal setup paragraphs in the simplest familiar setting that makes the actor's goal, real limitation, trigger, and observable cost clear. Its total length follows the complete causal chain rather than a fixed paragraph cap. The narrative runs the same bounded inputs/states, operations, results, and comparison later formalized compactly in the worked example, then returns to fact in <code>1 → 2 → 3 → 4</code> order. Both modes reject story vocabulary that needs a second explanation and distinguish exact relations from finite approximations when relevant. For daily briefings, the rank-1 academic paper remains the default story source unless an explicit override reason is recorded.
...
The briefing is not complete at candidate collection, Markdown, or config generation. The terminal deliverable opens with a validated concept fable, factual debrief, and default-collapsed concept-appropriate worked example, followed by the interactive HTML briefing, with default-<code>unrated</code> feedback JSON, manifest, and updated story index.
