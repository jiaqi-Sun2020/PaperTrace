...
`skills/allegory-teach` is an optional read-only explanation layer within Pipeline 4 and an authoring dependency for the Pipeline 2 opening story. Ordinary first-contact teaching defaults to Bridge Mode: disclose the technical problem, run a minimum concrete case, use one local analogy for one relation, map it, and return to formal language. Explicit allegory or delayed-reveal requests and every Pipeline 2 opening story use Fable Mode. Every newly authored fable begins with at least two immersive, causally necessary setup paragraphs; it then uses as many mechanism paragraphs as logical completeness requires, with no total paragraph cap. Mathematical or numerical stories must expose the actual inputs, operations, intermediate/result values, counterfactual, and observable difference in the narrative itself; non-mathematical stories use explicit states and transitions without decorative formulas. The daily briefing handoff keeps `opening_story.version=3`, accepts at least two paragraphs with a 2000-character per-paragraph fail-closed limit, and does not add an `explanation_mode` or background field; the briefing pipeline alone validates, renders, and publishes it. The skill must not replace `adaptive-teach` selection/session ownership, mutate the profile, alter news ranking, or treat analogy exposure as evidence.
