...
`skills/allegory-teach` is an optional read-only explanation layer within Pipeline 4 and an authoring dependency for the Pipeline 2 opening story. Technical correctness and beginner comprehensibility are both hard gates. Ordinary first-contact teaching defaults to Bridge Mode: disclose the technical problem and run a minimum concrete case, adding one local analogy only when it lowers cognitive load, then return to formal language. An explicit allegory request first attempts Fable Mode by narrowing to one relation and building coupled narrative and technical causal spines; if the repaired story still only renames variables or replays a formula, disclose the mismatch and use Bridge Mode. Every Pipeline 2 opening must remain Fable Mode: try the rank-1 academic item first, then use the existing explicit override in rank order only when the story-value gate fails. A valid fable begins with at least two causally necessary setup paragraphs, makes the actor's goal, real limit, choice or changed action, and observable consequence teach the mechanism, and uses as many paragraphs as causal completeness requires. Complete both spines before compression; there is no total word or paragraph cap. Mathematical or numerical stories must expose the actual inputs, operations, intermediate/result values, counterfactual, and observable difference in the narrative itself; non-mathematical stories use explicit states and transitions without decorative formulas. The factual return uses `1 -> 2 -> 3 -> 4`, followed by an unnumbered worked example that cannot supply a missing story spine after the fact. The daily briefing handoff keeps `opening_story.version=3`, accepts at least two paragraphs with a 2000-character per-paragraph fail-closed transport limit, and does not add an `explanation_mode` or background field; overlong content is split at natural causal boundaries, never truncated. The briefing pipeline alone validates, renders, and publishes it. The skill must not replace `adaptive-teach` selection/session ownership, mutate the profile, alter news ranking, or treat analogy exposure as evidence.
