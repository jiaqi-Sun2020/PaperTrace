...
`skills/allegory-teach` is an optional narrative explanation layer within Pipeline 4 and a read-only authoring dependency for the Pipeline 2 opening story. On a profile- or source-grounded request, it may turn one selected concept into a causal fable and factual debrief; the daily briefing pipeline alone validates, renders, and publishes its handoff. It must not replace `adaptive-teach` selection/session ownership, mutate the profile, alter news ranking, or treat story exposure as evidence.
